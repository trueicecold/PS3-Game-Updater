...
namespace PS3GameDetector
{
    class UpdateListData
    {
        ...
        public string GameName
        {
            get
            {
                return _updateGameName;
            }
        }

        public UpdateListData(string updateContent)
        {
                ...
                if (xmlParser.SelectSingleNode("//titlepatch").Attributes["status"].Value == "alive")
                {
                    ...
                    if (_updateVersions.Count > 0)
                    {
                        ...
                        for (int i = 0; i < _updateVersions.Count; i++)
                        {
                            ...
                            try
                            {
                                _updateGameName = xmlParser.SelectSingleNode("//titlepatch/tag/package/paramsfo/TITLE").InnerText;
                            }
                            catch (Exception ex)
                            {
                            }
